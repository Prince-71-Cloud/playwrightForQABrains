...
test('verify all navigation links and buttons functionality', async ({ page, context }) => {
    ...
    const [qaTopicsPage] = await Promise.all([
      ...
      qaTopicsLink.click(),
        ...
        const discussionLink = page.locator('#nav').getByRole('link', { name: 'Discussion' });
        ...
        const practiceSiteLink = page.locator('#nav').getByRole('link', { name: 'Practice Site' });
        ...
        const aboutUsLink = page.locator('#nav').getByRole('link', { name: 'About Us' });
